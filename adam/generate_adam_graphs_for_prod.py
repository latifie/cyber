...
def main():
    ...
    if len(top_tlds) > 0:
        ...
        if "Militarisation Directe" in tld_dist_pct_fixed.columns:
            tld_dist_pct_fixed = tld_dist_pct_fixed.sort_values(by="Direct Weaponization", ascending=False)
    ...
    if len(top_10_regs) > 0:
        ...
        if "Direct Weaponization" in reg_dist_pct.columns:
            reg_dist_pct = reg_dist_pct.sort_values(by="Direct Weaponization", ascending=False)
    ...
    plt.yscale("symlog")
    plt.title("Network Infrastructure Complexity (Unique IPs)")
    plt.xlabel("Strategic Age Cluster")
    ...
    if not old_incubated.empty:
        def categorize_drop(ud_val):
            if pd.isna(ud_val) or ud_val < 0: return "Unknown / No Update"
            if ud_val <= 3: return "Drop Catching (Buyback, Update < 3d)" 
            return "Classic Incubation"
