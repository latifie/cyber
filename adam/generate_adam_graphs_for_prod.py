...
def main():
    ...
    with open(latest_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Filter unrealistic or negative values for aging
            if aging_time < 0:
            ...
            # Update Delay: Must be positive (UD should be before discovery)
            update_delay = None
            if ud:
                update_delay = (discovery - ud).total_seconds() / 86400
                if update_delay < 0:
                    filter_stats["neg_update"] += 1
                    update_delay = None # We ignore the negative delay for the scatter plot
            ...
            uptime_dur = takedown.get("uptime_dur") # in hours
            ...
            for u in rec.get("uptime", []):
                ...
                # Logic Fix: We allow dt == discovery and slight precision errors
                # But we strictly look for signals BEFORE or AT the moment of weaponization
    ...
    # ---------------------------------------------
    # Thème 1 / Ch 1 / Cl 1 : Global distribution
    # ---------------------------------------------
    plt.figure()
    ...
    # ---------------------------------------------
    # Thème 2 / Ch 8 / Cl 7 : Update Delay
    # ---------------------------------------------
    # Filtrage strict des valeurs valides et positives uniquement
    ...
    # ---------------------------------------------
    # Thème 3 / Ch 5 / Cl 5 : Registrar Profiling
    # ---------------------------------------------
    ...
    # ---------------------------------------------
    # Thème 4 / Ch 6 / Cl 2 : Target / Brand Value
    # ---------------------------------------------
    ...
    if len(df_targets) > 0:
        ...
        ax = target_stats.plot(kind="bar", figsize=(12, 6), color=["#34495e", "#e67e22"])
        plt.yscale("log") # Recommendation 5: Log scale for targets
    ...
    # ---------------------------------------------
    # Thème 5 / Ch 10 / Cl 3 : Temporal Evolution
    # ---------------------------------------------
    plt.figure()
    yearly_median = df.groupby("year")["aging_time"].median().reset_index()
    # Recommendation 3: Disable strict filter for sample data
    ...
    if len(yearly_median) > 0:
        ...
        # Recommendation 3: Force full years display
    ...
    if len(df_surv) > 0:
        plt.figure()
        # Recommendation 2: Replace regplot with scatterplot to avoid negative regression lines
